...
add_filter(
	...
	function ( $exporters ) {
		$exporters['teleborgsvvs-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'teleborgsvvs-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'teleborgsvvs-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'teleborgsvvs-leads',
						'group_label' => __( 'Quote requests', 'teleborgsvvs-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['teleborgsvvs-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'teleborgsvvs-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
