...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'teleborgsvvs-leads' ),
					'singular_name' => __( 'Lead', 'teleborgsvvs-leads' ),
					'edit_item'     => __( 'Förfrågan', 'teleborgsvvs-leads' ),
					'search_items'  => __( 'Search leads', 'teleborgsvvs-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'teleborgsvvs-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'teleborgsvvs-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'teleborgsvvs-leads' ),
			'npl_phone'    => __( 'Phone', 'teleborgsvvs-leads' ),
			'npl_service'  => __( 'Needs', 'teleborgsvvs-leads' ),
			'npl_timeline' => __( 'When', 'teleborgsvvs-leads' ),
			'npl_zip'      => __( 'ZIP', 'teleborgsvvs-leads' ),
			'npl_source'   => __( 'Source', 'teleborgsvvs-leads' ),
			'npl_status'   => __( 'Status', 'teleborgsvvs-leads' ),
			'date'         => __( 'Received', 'teleborgsvvs-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'teleborgsvvs-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'teleborgsvvs-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'teleborgsvvs-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'teleborgsvvs-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'teleborgsvvs-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'teleborgsvvs-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'teleborgsvvs-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'teleborgsvvs-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'teleborgsvvs-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'teleborgsvvs-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'teleborgsvvs-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'teleborgsvvs-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
